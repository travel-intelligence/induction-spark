...
object Utils {
  ...
  private[ds] def loadTrain(dataFile: String): Array[ByteRecord] = {
    val allFiles = Array(
  ...
  private[ds] def loadTest(dataFile: String): Array[ByteRecord] = {
    val result = new ArrayBuffer[ByteRecord]()
   ...
   * @param result
   */
  private[ds] def load(featureFile: String, result : ArrayBuffer[ByteRecord]): Unit = {
    val rowNum = 32
    val colNum = 32
